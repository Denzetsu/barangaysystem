...
if (isset($_POST['edit'])) {
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        $result = mysqli_query($conn, "SELECT `image` FROM `projects2` WHERE `user_id` = '$user_id'");
        ...
        if (in_array($end, $allowed_ext)) {
            ...
            if (file_exists($previous) && unlink($previous)) {
                if (move_uploaded_file($image_temp, $path)) {
                    mysqli_query($conn, "UPDATE `projects2` SET `title` = '$title', `details` = '$details',  `budget` = '$budget',  `date` = '$date', `image` = '$path' WHERE `user_id` = '$user_id'") or die(mysqli_error($conn));
        ...
        mysqli_query($conn, "UPDATE `projects2` SET `title` = '$title', `details` = '$details',  `budget` = '$budget',  `date` = '$date' WHERE `user_id` = '$user_id'") or die(mysqli_error($conn));
...
$query = "DELETE FROM `projects2` WHERE `user_id` = '$id'";

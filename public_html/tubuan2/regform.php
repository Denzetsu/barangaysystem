  ...
  if (isset($_POST['submit'])) {
    ...
    if ($_POST['verification_code'] == $_SESSION['verification_code']) {
      ...
      if ($emailExists) {
        ...
        $sql = "INSERT INTO $table (first_name, middle_name, last_name, suffix_name, gender, address, date_of_birth, birth_place, occupation, voter_status, barangay, email, password, family_head) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        if ($stmt = $conn->prepare($sql)) {
          // Bind parameters
          $stmt->bind_param("ssssssssssssss", $fname, $mname, $lname, $sname, $gender, $address, $dateofbirth, $Bplace, $Work, $Vstatus, $Town, $Email, $Pword, $Fhead);
      ...
      <div class="column">
        ...
        <div class="input-box">
          ...
          <select class="select-box" name="Town" required>
            ...
            <option value="Tubuan II" <?php echo (isset($_POST['Town']) && $_POST['Town'] == 'Tubuan II') ? 'selected' : ''; ?>>Tubuan II</option>
            </select>
        </div>

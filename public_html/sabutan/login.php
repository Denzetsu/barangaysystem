...
if (isset($_POST['login'])) {
    ...
    if ($result_sabutan->num_rows == 1) {
        ...
        header("Location: residentspage1.php");
        exit();
    } elseif ($result_sabutan->num_rows == 1) {
        // Store user ID in a session variable
        $_SESSION['user_id'] = $result_sabutan->fetch_assoc()['id'];
        $_SESSION['email'] = $email;
        // Redirect to user_page3.php for users in sabutan
        header("Location: residentspage1.php");
        exit();
    } else {
        // Check if the email exists but password is incorrect
        $sql_check_email = "SELECT * FROM (SELECT * FROM sabutan UNION SELECT * FROM sabutan UNION SELECT * FROM sabutan) AS all_users WHERE email = '$email'";
